  ...
  (function initMultiPage() {
    const run = () => {
      ...
      if (typeof _initBrandModal === 'function') _initBrandModal();
    };
  ...
  const BRAND_SLIDES = [
    {
      lead: '우리는 아이디어 기반의 디자인 그룹입니다.',
      body:
        '<strong>Another Movement</strong>는 기업과 스타트업을 위한 아이디어 기반 디자인 그룹입니다.<br>' +
        '단순히 보이는 것을 다루기보다, 보이지 않는 생각의 구조를 설계합니다.<br>' +
        '아이디어의 가능성을 발굴하고, 그것이 실현 가능한 방향성을 갖도록 전략적으로 구체화합니다.<br>' +
        '비즈니스, 브랜드, 사용자의 맥락을 함께 고려하며,<br>' +
        '단기적인 시각물보다 장기적으로 작동하는 설계와 구조에 집중합니다.<br>' +
        '기획 중심의 사고로 브랜드의 본질을 함께 고민하며,<br>' +
        '디자인 이전의 질문부터 치열하게 탐구합니다.'
    },
    {
      lead: '디자인은 우리의 사고방식입니다.',
      body:
        '우리에겐 디자인은 단지 결과물이 아닌, 사유의 언어이자 전략의 형태입니다.<br>' +
        '<strong>Another Movement</strong>는 디자인을 단순한 시각적 장식이 아닌,<br>' +
        '문제 해결과 의미 전달의 체계적 수단으로 접근합니다.<br>' +
        '기획, 리서치, 워딩, 구조화, 시각화까지—<br>' +
        '우리는 디자인의 전 과정을 통합적 사고로 조율합니다.<br>' +
        '명확하게 말할 수 없는 브랜드는 설계될 수 없기에,<br>' +
        '우리는 디자인 이전에 먼저 구조를 정리하고, 설득 가능한 언어로 정제합니다.<br>' +
        '그 위에 구축된 시각적 표현은 명확하고, 지속적으로 작동합니다.'
    },
    {
      lead: '아이디어가 디자인이 되고, 디자인이 브랜드가 됩니다.',
      body:
        '<strong>Another Movement</strong>는 아이디어와 디자인, 브랜드를 분리해서 보지 않습니다.<br>' +
        '하나의 생각이 구조를 갖추고, 그 구조가 시각 언어로 번역될 때<br>' +
        '브랜드는 표면이 아닌 본질에서 형성된다고 믿습니다.<br>' +
        '우리는 아이디어의 가능성을 발굴해 디자인의 논리로 정리하고,<br>' +
        '그 위에 일관된 메시지와 경험을 쌓아 브랜드로 확장합니다.<br>' +
        '브랜드는 ‘만드는 것’이 아니라 ‘발현되는 것’—<br>' +
        '디자인은 그 발현을 돕는 정교한 도구이자, 아이디어를 현실로 잇는 다리입니다.<br>' +
        '정체성, 구조, 메시지, 경험까지 우리는 생각에서 브랜드까지를 하나의 흐름으로 설계합니다.'
    },
    {
      lead: '우리는 움직이는 디자인을 만듭니다.',
      body:
        '<strong>Another Movement</strong>는 단기적인 트렌드를 따르지 않습니다.<br>' +
        '우리는 브랜드와 사용자가 계속해서 연결되고 반응하는 시스템을 설계합니다.<br>' +
        '그 움직임은 결과물에만 머물지 않고,<br>' +
        '사람들의 인식과 행동을 유도하는 방향성으로 이어집니다.<br>' +
        '우리가 만드는 것은 단순한 비주얼이 아닌,<br>' +
        '지속 가능한 구조와 의미 있는 흐름입니다.<br><br>' +
        '<strong>Another Movement.</strong><br>' +
        '디자인으로 세상을 움직이게 하는 힘.'
    }
  ];
  let _brandIdx = 0;

  function _ensureBrandModalDom() {
    if (document.getElementById('brandModal')) return;
    const style = document.createElement('style');
    style.id = 'brandModalStyle';
    style.textContent =
      '.brand-modal{position:fixed;inset:0;z-index:1200;display:none;align-items:center;justify-content:center;padding:24px;background:rgba(17,17,17,.55);}' +
      '.brand-modal.open{display:flex;}' +
      '.brand-card{position:relative;width:100%;max-width:620px;max-height:88vh;overflow-y:auto;background:#fff;border-radius:14px;padding:48px 44px 32px;box-shadow:0 24px 70px rgba(0,0,0,.22);animation:brandIn .35s ease;}' +
      '@keyframes brandIn{from{opacity:0;transform:translateY(14px);}to{opacity:1;transform:none;}}' +
      '.brand-close{position:absolute;top:16px;right:18px;width:34px;height:34px;border-radius:50%;font-size:20px;line-height:1;color:#111;background:transparent;display:flex;align-items:center;justify-content:center;transition:background .15s;}' +
      '.brand-close:hover{background:#f0f0f0;}' +
      '.brand-logo-img{display:block;height:46px;width:auto;margin-bottom:30px;}' +
      'body.dark .brand-logo-img{filter:invert(1);}' +
      '.brand-lead{font-size:15px;font-weight:600;color:#111;margin-bottom:22px;}' +
      '.brand-body{font-size:14px;line-height:2;color:#222;min-height:230px;}' +
      '.brand-body strong{font-weight:700;}' +
      '.brand-foot{display:flex;align-items:center;justify-content:space-between;margin-top:26px;}' +
      '.brand-dots{display:flex;gap:9px;}' +
      '.brand-dots b{width:9px;height:9px;border-radius:50%;border:1px solid #111;background:transparent;cursor:pointer;transition:background .15s;}' +
      '.brand-dots b.on{background:#111;}' +
      '.brand-nav{display:flex;gap:8px;}' +
      '.brand-nav button{font-size:13px;color:#555;padding:6px 12px;border-radius:8px;transition:background .15s,color .15s;}' +
      '.brand-nav button:hover{background:#f2f2f2;color:#111;}' +
      '.brand-nav button:disabled{opacity:.3;cursor:default;background:transparent;}' +
      '.footer-bottom .brand-link{cursor:pointer;text-decoration:none;transition:opacity .15s;}' +
      '.footer-bottom .brand-link:hover{opacity:.55;text-decoration:underline;}' +
      'body.dark .brand-card{background:#1a1a1a;}' +
      'body.dark .brand-logo,body.dark .brand-lead,body.dark .brand-close{color:#f5f5f5;}' +
      'body.dark .brand-body{color:#d6d6d6;}' +
      'body.dark .brand-dots b{border-color:#f5f5f5;}body.dark .brand-dots b.on{background:#f5f5f5;}' +
      'body.dark .brand-close:hover{background:#2a2a2a;}body.dark .brand-nav button:hover{background:#2a2a2a;color:#fff;}' +
      '@media(max-width:520px){.brand-card{padding:40px 24px 26px;}.brand-logo-img{height:36px;}.brand-body{font-size:13px;}}';
    ...
    modal.id = 'brandModal';
    modal.innerHTML =
      '<div class="brand-card" role="dialog" aria-modal="true" aria-label="Another Movement 브랜드 스토리">' +
        '<button class="brand-close" id="brandClose" aria-label="닫기">&times;</button>' +
        '<img class="brand-logo-img" src="/assets/img/anothermovement_logo.png" alt="Another Movement">' +
        '<div class="brand-lead" id="brandLead"></div>' +
        '<div class="brand-body" id="brandBody"></div>' +
        '<div class="brand-foot">' +
          '<div class="brand-dots" id="brandDots"></div>' +
          '<div class="brand-nav">' +
            '<button id="brandPrev" type="button">이전</button>' +
            '<button id="brandNext" type="button">다음</button>' +
          '</div>' +
        '</div>' +
      '</div>';
    ...
    document.getElementById('brandPrev').addEventListener('click', () => _brandGo(_brandIdx - 1));
    document.getElementById('brandNext').addEventListener('click', () => _brandGo(_brandIdx + 1));
    document.addEventListener('keydown', (e) => {
      if (!modal.classList.contains('open')) return;
      if (e.key === 'Escape') closeBrandModal();
      else if (e.key === 'ArrowLeft') _brandGo(_brandIdx - 1);
      else if (e.key === 'ArrowRight') _brandGo(_brandIdx + 1);
    });
  }

  function _brandRender() {
    const s = BRAND_SLIDES[_brandIdx];
    const lead = document.getElementById('brandLead');
    const body = document.getElementById('brandBody');
    const dots = document.getElementById('brandDots');
    if (!s || !lead) return;
    lead.textContent = s.lead;
    body.innerHTML = s.body;
    dots.innerHTML = BRAND_SLIDES.map((_, i) =>
      '<b class="' + (i === _brandIdx ? 'on' : '') + '" data-i="' + i + '"></b>').join('');
    dots.querySelectorAll('b').forEach(b =>
      b.addEventListener('click', () => _brandGo(parseInt(b.dataset.i, 10))));
    document.getElementById('brandPrev').disabled = _brandIdx === 0;
    document.getElementById('brandNext').disabled = _brandIdx === BRAND_SLIDES.length - 1;
    const card = document.querySelector('#brandModal .brand-card');
    if (card) card.scrollTop = 0;
  }

  function _brandGo(i) {
    if (i < 0 || i >= BRAND_SLIDES.length) return;
    _brandIdx = i;
    _brandRender();
  }

  function openBrandModal() {
    _ensureBrandModalDom();
    _brandIdx = 0;
    _brandRender();
  ...
  function _initBrandModal() {
    document.querySelectorAll('.footer-bottom span').forEach(sp => {
      ...
      if ((sp.textContent || '').toUpperCase().includes('CM LAB')) {
        ...
        sp.addEventListener('click', openBrandModal);
  ...
  /* ===== Sentence collection (drag/long-press to collect) ===== */
  let collectingArticleId = null;
  let _pendingSelectionText = '';
  let _selChangeTimer = null;
  const _isTouch = ('ontouchstart' in window) || (window.matchMedia && window.matchMedia('(pointer:coarse)').matches);
  ...
  function placeSelectionPopover() {
    ...
    const body = node?.closest?.('.article-body');
    if (!body) return false;
    const articleEl = body.closest('.article-content');
    if (!articleEl) return false;
    collectingArticleId = articleEl.id.replace(/^article-/, '');
    _pendingSelectionText = text;
    const p = document.getElementById('selectionPopover');
    if (_isTouch) {
      // Mobile: fixed bottom-center floating action (avoids native selection toolbar)
  ...
  function hideSelectionPopover() {
    ...
    // NOTE: do NOT clear _pendingSelectionText here — keep it so a tap right after
    // can still save even if selection collapsed mid-tap on mobile.
  }
  function collectSelection() {
    const text = _pendingSelectionText;
    if (!text || !collectingArticleId) return;
    if (!isLoggedIn()) {
      _pendingSelectionText = '';
      hideSelectionPopover();
      showToast('로그인 후 문장을 수집할 수 있어요.');
      setTimeout(() => openLogin(), 500);
      return;
    }
    const sentences = getSentences();
    const d = new Date();
    const dateStr = `${d.getFullYear()}.${String(d.getMonth()+1).padStart(2,'0')}.${String(d.getDate()).padStart(2,'0')}`;
    const _collectMeta = _lookupArticle(collectingArticleId) || (window.__currentArticle && window.__currentArticle.id === collectingArticleId ? window.__currentArticle : null);
    const _collectTitle = _collectMeta ? (_collectMeta.title || '') : '';
    sentences.unshift({ text, articleId: collectingArticleId, title: _collectTitle, date: dateStr, ts: Date.now() });
    saveSentences(sentences);
    const savedArticleId = collectingArticleId;
    _pendingSelectionText = '';
    collectingArticleId = null;
    hideSelectionPopover();
    try { window.getSelection()?.removeAllRanges(); } catch(e) {}
    // Apply highlight immediately to the current article
    if (savedArticleId) applyCollectedHighlights(savedArticleId);
    showToast('문장을 수집했어요.<br/>마이페이지에서 다시 볼 수 있어요.');
  }
  function citeSelection() {
    ...
    const finish = () => {
      ...
      hideSelectionPopover();
      try { window.getSelection()?.removeAllRanges(); } catch(e) {}
      showToast('인용 형식으로 복사했어요.<br/>붙여넣기로 사용하세요.');
    };
    if (navigator.clipboard) {
      navigator.clipboard.writeText(citation).then(finish, finish);
    } else {
      finish();
    }
  }

  // Selection events — covers both mouse and touch
  document.addEventListener('mouseup', () => {
    setTimeout(() => {
      if (!placeSelectionPopover()) hideSelectionPopover();
    }, 10);
  });
  document.addEventListener('touchend', () => {
    setTimeout(() => {
      placeSelectionPopover();
    }, 350); // wait for native selection to settle
  }, { passive: true });
  document.addEventListener('selectionchange', () => {
    // On touch: don't hide popover when selection collapses (user tapping popover causes collapse)
    if (_isTouch) {
      clearTimeout(_selChangeTimer);
      _selChangeTimer = setTimeout(() => {
        const sel = window.getSelection();
        if (sel && !sel.isCollapsed) placeSelectionPopover();
      }, 350);
      return;
    }
    const sel = window.getSelection();
  ...
  document.addEventListener('scroll', hideSelectionPopover, { passive: true });

  // Mobile: tap outside (not popover, not article) → hide
  ...
  // 선택 팝오버 DOM이 페이지에 없으면 동적으로 생성(모든 글 페이지 공통)
  function ensureSelectionPopover() {
    let p = document.getElementById('selectionPopover');
    if (p) return p;
    p = document.createElement('div');
    p.id = 'selectionPopover';
    p.className = 'selection-popover';
    p.setAttribute('role', 'toolbar');
    p.innerHTML =
      '<button type="button" class="sp-action sp-collect"><i class="fa-solid fa-bookmark"></i>수집</button>' +
      '<button type="button" class="sp-action sp-cite"><i class="fa-solid fa-quote-right"></i>인용</button>';
    document.body.appendChild(p);
    return p;
  }

  // popover triggers — collect or cite (works on both click and touch)
  (function() {
    const p = ensureSelectionPopover();
    if (!p) return;
    // prevent selection collapse from desktop mousedown anywhere in popover
    p.addEventListener('mousedown', e => { e.preventDefault(); });
    function runAction(target) {
      if (target.closest('.sp-cite')) citeSelection();
      else if (target.closest('.sp-collect')) collectSelection();
    }
